...
module namespace cry="http://www.smls.com.br/CryptoSmls";

declare function cry:decryptBase64($base64Text as xs:string) as xs:integer* {
    let $codepoints :=  string-to-codepoints($base64Text) 
...
declare function cry:encryptBase64($input as xs:string*) as xs:integer* {
    let $b := fn:string-to-codepoints($input)
    let $table := ( 65 to  90,  (: A-Z :)
                    97 to 122,  (: a-z :)
                    48 to  57,  (: 0-9 :)
                    43, 47 )    (: + / :)
